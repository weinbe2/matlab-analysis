...
short_runname=`echo $runname | sed -e s/gcr//`

run=${PBS_JOBNAME}
fname=`echo $run | sed -e s/.sh// -e s/gcr//`
...
cmd="${BINARY} -c ${input} -i /lqcdproj/LSD/8f/${short_runname}/configs/config.${config}.lime -o f4plus8l24t48b40m010m080_38000_scidac_gfix"

echo $cmd

# Output filename doesn't matter!
${BINARY} -c ${input} -i /lqcdproj/LSD/8f/${short_runname}/configs/config.${config}.lime -o f4plus8l24t48b40m010m080_38000_scidac_gfix
...
flat=""
for f in ./output/${short_runname}_${config}_*
do
  flat="${flat} ${f}"
  repl=$(echo ${f} | sed -e "s/output/binned/g" -e "s/dat/bin/g")
  ./bin_corr_single.sh $f $repl
done
...
tar -cvzf ./output/${short_runname}_${config}.tar.gz2 ${flat}
